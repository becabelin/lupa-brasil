  ...
  const results = useMemo(() => {
    ...
    return analyses
      .map((a) => {
        ...
        const hay = [topic.summary, ...topic.proposals, ...topic.quotes]
          .join(" ")
          .toLowerCase();
      ...
      }) as { candidate: Candidate; topic: PlanAnalysis["topics"][number] }[];
  }, [analyses, candidates, topicId, query]);
        ...
        <div className="space-y-3">
          {results.map(({ candidate, topic }) => (
            ...
            >
              ...
              <p className="mt-3 text-sm opacity-80">{topic.summary}</p>
            </Link>
          ))}
        </div>
      )}
